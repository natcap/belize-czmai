...
REC outputs a shapefile map for each scenario (2016 = baseline, 2020 = current, and 2025 = informed management), which shows the visitation patterns in phot-users days across the area of interest. We need to combine those shapefiles to (1) calculate the change in visitation compared to 2016 and (2) summarize the change by coastal planning units.

Set our working directory towards the folder with the REC outputs in our shared Gdrive:
...
rec_output_shp_2016_all <- st_read("E:/Greenfin/03_recreation/rec_outputs/habitats_2016/pud_results_2016_2020.shp")
rec_output_shp_2020_all <- st_read("E:/Greenfin/03_recreation/rec_outputs/habitats_2016/scenario_results_2016_2020.shp")
rec_output_shp_2025_all <- st_read("E:/Greenfin/03_recreation/rec_outputs/habitats_2016/scenario_results_2016_2025.shp")
```
...
Calculate the percent and number of overnight and cruise visitors for 2016:
...
Calculate the percent and number of overnight and cruise visitors for 2020:
...
Calculate the percent and number of overnight and cruise visitors for 2025:
...
Calculate the number of overnight and cruise visitors for 2016, 2020 and 2025
...
calculate the annual expenditure per visitor type and for the total number of person-days for 2016,2020, snd 2025:
...
class(rec_output_sp)
writeOGR(obj=rec_output_sp, dsn="E:/Greenfin/03_recreation/rec_outputs/habitats_2016", layer="rec_output", driver="ESRI Shapefile", overwrite_layer =  T)
rec_output <- as.data.frame(rec_output_sp)
names(rec_output)
write.csv(rec_output, file.path("E:/Greenfin/03_recreation/rec_outputs/habitats_2016", paste("rec_summary.csv")), row.names = F)
```

Summarize results by coastal planning units & add the CPU names:
```{r Summarize results by coastal planning units}
rec_output$NAME <- as.factor(rec_output$NAME)
rec_output$NEW_NAME <- as.factor(rec_output$NEW_NAME)
rec_output_by_cpu <- aggregate(rec_output[,-c(1:6)],rec_output["NAME"],sum)
...
CPU <- CPU_df[,c(1,3)]
CPU$CPU <- paste(CPU$NAME,CPU$NEW_NAME,sep= "-")
rec_output_by_cpu <- cbind(rec_output_by_cpu,CPU, by="NAME")
rec_output_by_cpu <- rec_output_by_cpu[, c(1,20:21,2:18)]
write.csv(rec_output_by_cpu, file.path("E:/Greenfin/03_recreation/rec_outputs/habitats_2016", paste("rec_summary_by_cpu.csv")), row.names = F)
```
...
rec_output_by_cpu_tpd_2016_flip <- melt(rec_output_by_cpu_tpd_2016, id=c("NAME","NEW_NAME","CPU","Year")) 
...
rec_output_by_cpu_tpd_2020_flip <- melt(rec_output_by_cpu_tpd_2020, id=c("NAME","NEW_NAME","CPU","Year")) 
...
rec_output_by_cpu_tpd_2025_flip <- melt(rec_output_by_cpu_tpd_2025, id=c("NAME","NEW_NAME","CPU","Year")) 
...
write.csv(rec_output_by_cpu_tpd, file.path("E:/Greenfin/03_recreation/rec_outputs/habitats_2016", paste("rec_summary_by_cpu_tpd.csv")), row.names = F)
```
...
rec_output_by_cpu_usd_2016_flip <- melt(rec_output_by_cpu_usd_2016, id=c("NAME","NEW_NAME","CPU","Year")) 
...
rec_output_by_cpu_usd_2020_flip <- melt(rec_output_by_cpu_usd_2020, id=c("NAME","NEW_NAME","CPU","Year")) 
...
rec_output_by_cpu_usd_2025_flip <- melt(rec_output_by_cpu_usd_2025, id=c("NAME","NEW_NAME","CPU","Year")) 
...
write.csv(rec_output_by_cpu_usd, file.path("E:/Greenfin/03_recreation/rec_outputs/habitats_2016", paste("rec_summary_by_cpu_usd.csv")), row.names = F)
```
...
rec_outputs <- read.csv("E:/Greenfin/03_recreation/rec_outputs/habitats_2016/rec_summary_by_cpu_tpd.csv")
...
rec_outputs$CPU <- factor(rec_outputs$CPU,  # Change ordering manually
...
ggbars <- ggplot(rec_outputs) +
  geom_bar(aes(x=CPU, y=TPD, fill=Year), position="dodge", stat="identity")+
...
pdf(file.path("E:/Greenfin/03_recreation/rec_outputs/habitats_2016", paste("_barplot_cpu_rec_outputs_tpd.pdf")),
    width = 8, height = 5)
plot_grid(ggbars + coord_flip() + scale_y_continuous(labels = scales::comma))
dev.off()
```
...
rec_outputs <- read.csv("E:/Greenfin/03_recreation/rec_outputs/habitats_2016/rec_summary_by_cpu_usd.csv")
rec_outputs$REC.model <- factor(rec_outputs$REC.model,
...
names(year_cols) <- c("2016", "2020", "2025")
ggbars <- ggplot(rec_outputs) +
  geom_bar(aes(x=CPU, y=USDx10000, fill=Year), position="dodge", stat="identity") +
  scale_fill_manual(values=year_cols) +
  ...
  theme(axis.title.x = element_text(size = 12, face="bold"), 
        ...
        legend.position="bottom")
plot_grid(ggbars + coord_flip() + scale_y_continuous(labels = scales::comma))
pdf(file.path("E:/Greenfin/03_recreation/rec_outputs/habitats_2016", paste("_barplot_cpu_rec_outputs_usd.pdf")),
    width = 8, height = 5)
plot_grid(ggbars + coord_flip() + scale_y_continuous(labels = scales::comma))
dev.off()
```
